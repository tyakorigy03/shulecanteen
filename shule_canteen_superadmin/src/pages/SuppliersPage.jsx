...
const SuppliersPage = () => {
    ...
    useEffect(() => {
        const q = query(collection(db, 'onboarding_requests'), where('entityType', '==', 'supplier'));
    ...
    const filteredSuppliers = suppliers.filter(s => {
        ...
        const nameMatch = (s?.companyName || s?.name || '').toLowerCase().includes(search);
    ...
    return (
        <div className="flex flex-col gap-6 font-outfit text-white">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div>
                    <h2 className="text-3xl font-black">Supplier <span className="text-shuleamber">Hub</span></h2>
                    <p className="text-sm opacity-60 mt-1">Onboard and manage food vendors & supply contracts</p>
                </div>
                <Link
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                <div className="p-4 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
                    <span className="text-[10px] font-bold text-navblue/60 uppercase tracking-widest">Supplier Registry ({filteredSuppliers.length})</span>
                ...
                <div className="overflow-x-auto min-h-[400px]">
                    <table className="w-full text-left text-navblue text-xs relative">
                        <thead className="bg-navblue/5 border-b border-gray-100 uppercase text-[9px] tracking-wider text-navblue/40 font-bold">
                            <tr>
                                <th className="px-4 sm:px-6 py-4">Supplier Info</th>
                                <th className="hidden sm:table-cell px-6 py-4">Category</th>
                                <th className="hidden md:table-cell px-6 py-4">Primary Contact</th>
                                ...
                                <tr key={sup.id} className="hover:bg-gray-50 transition-colors group">
                                    <td className="px-4 sm:px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="font-bold text-[11px] sm:text-xs leading-tight">{sup.companyName || sup.name}</span>
                                            <span className="text-[9px] opacity-40 font-black uppercase tracking-tighter mt-1">{sup.id}</span>
                                        </div>
                                    </td>
                                    <td className="hidden sm:table-cell px-6 py-4">
                                        <div className="flex items-center gap-2 italic font-medium opacity-70">
                                            <HiOutlineLocationMarker className="w-3.5 h-3.5" />
                                            <span>{sup.district}, {sup.province}</span>
                                        </div>
                                    </td>
                                    <td className="hidden md:table-cell px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="font-bold">{sup.fullName || sup.contact || 'No Name'}</span>
                                            <span className="text-[10px] opacity-40">{sup.phone}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 sm:px-6 py-4 text-right">
                                        <span className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[8px] sm:text-[9px] font-bold uppercase tracking-wider ${sup.status === 'active' || sup.status === 'Active' ? 'text-green-500 bg-green-50' : sup.status === 'rejected' ? 'text-red-500 bg-red-50' : 'text-blue-500 bg-blue-50'
                                            }`}>
                                            {sup.status}
                                        </span>
                                    </td>
                                    <td className="px-4 sm:px-6 py-4 text-right relative">
                                        ...
                                        {openMenuId === sup.id && (
                                            <div className="absolute right-12 top-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-[60] animate-in fade-in slide-in-from-right-2 duration-200 text-left">
                                                ...
                                                <button
                                                    onClick={() => handleStatusUpdate(sup.id, 'active')}
                                                ...
                                                <button
                                                    onClick={() => handleStatusUpdate(sup.id, 'rejected')}
